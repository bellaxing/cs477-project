function main() {
  ...
  // let orderH = false;
   ...
   window.onload = function () {
     ...
     document.getElementById("creat_new_bank_Account_id").onclick =
       displayBankForm;
     document.getElementById("bank_account_submit").onclick =
   ...
   function displayToClient(bookDisplay, data) {
     ...
     viewBtn.addEventListener("click", function () {
       ...
       async function getItem() {
         //cart=false;
         let cart1 = await (
           await fetch("http://localhost:3000/books/" + data._id, {
             headers: {
               Authorization: "Bearer " + sessionStorage.getItem("token"),
             },
           })
   ...
   async function createNewBankAccount() {
     ...
     if (
       ...
       let dateEx =
         cardExpDate.getMonth() +
         "/" +
         cardExpDate.getDate() +
         "/" +
         cardExpDate.getFullYear();
       document.getElementById("r_card_number").innerHTML =
         "Card number: " + cardNumber1;
       document.getElementById("r_card_expiry_date").innerHTML =
         "Expiry date: " + dateEx;
       document.getElementById("r_card_security_code").innerHTML =
         "Security code: " + accountInfo.csv;
     }
   }
   ...
   async function addNewUser() {
     ...
     if (
       ...
       if (result.exist) {
         ...
         document.getElementById("user_name_id").value = "user name must be unique";
       } else {
         loginDisplay();
       }
     }
   ...
   async function displayCartItem() {
     // let divH=document.getElementById('cart_display_form')
     // $('#divH').remove();
     totalPrice = 0;
     const item = await (
   ...
   async function getOrderHistory() {
     ...
     if (orderedHist.length == !0) {
       displayOrderHistoryF();
       let tBody = document.getElementById("o_table_body_id");
       for (let ord of orderedHist) {
         ord.ordered.forEach((data) => displayOrderHistory(data, tBody));
       }
     } else {
       alert("Empty order history");
     }
   }
 
   function displayOrderHistory(data, tBody) {
     let priceValue = document.getElementById("o_total_price_id");
     const tr = document.createElement("tr");
     const titleTd = document.createElement("td");
     titleTd.textContent = data.title;
     tr.appendChild(titleTd);
